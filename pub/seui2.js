...
(function(global) {
  ...
  function Seui(svgSpecifier, toAnimateId, animationsSpecs, defaultSpecs) {
    const _self = {};
    ...
    _self.animate = _self.svg.querySelector(`#${toAnimateId}`);
    if (!_self.animate) {
      console.error("The specifier for the element to animate is not valid.");
      return null;
    }
    _self.animateId = toAnimateId;
    ...
    _self.renderAnimation = () => {
      const toRemove = _self.animate
        .querySelectorAll(`[id^=${seui2AnimateMotionId}-${_self.animateId}]`);
  ...
  function createAndAppendAnimation(_self, index) {
    ...
    if (!_self.animate.querySelector(
      `#${seui2AnimateMotionId}-${_self.animateId}${index}`
    )) {
  ...
  function removeAnimation(_self, index, onEnd) {
    ...
    const nodeToRemove = _self.animate.querySelector(
      `#${seui2AnimateMotionId}-${_self.animateId}${index}`
    );
    nodeToRemove.remove();
  }
  ...
  function createMotionAnimation(specs) {
    ...
    animMotion.setAttribute('path', specs.path);
    animMotion.setAttribute('dur', specs.dur)
  ...
  */
  function getFullSpecs(_self, index) {
    ...
    _self.animationsSpecs[index].adjustedStart = fullSpecs.start;
    ...
    _self.animationsSpecs[index].adjustedEnd = fullSpecs.end;
    ...
    if (fullSpecs.begin.startsWith('.end')) {
      ...
      fullSpecs.begin = `${seui2AnimateMotionId}-${_self.animateId}${prevIndex}${fullSpecs.begin}`;
    }

    fullSpecs.id = `${seui2AnimateMotionId}-${_self.animateId}${index}`;
    ...
    if (toAvoid) {
      ...
      fullSpecs.path = generatePath(adjustedStart, adjustedEnd,
        fullSpecs.direction, borders, fullSpecs.bend);
    } else {
      fullSpecs.path = `m ${fullSpecs.start.x} ${fullSpecs.start.y} L ${fullSpecs.end.x} ${fullSpecs.end.y}`
    }

    const dummyPathNode = document.createElementNS("http://www.w3.org/2000/svg", "path");
    dummyPathNode.setAttribute('d', fullSpecs.path);
    if (specific.dur) fullSpecs.dur = specific.dur;
    else if (specific.speed) fullSpecs.dur = (dummyPathNode.getTotalLength() / specific.speed) + 's';
    else if (defaultSpecs.dur) fullSpecs.dur = defaultSpecs.dur;
    else if (defaultSpecs.speed) fullSpecs.dur = (dummyPathNode.getTotalLength() / defaultSpecs.speed) + 's';
    else fullSpecs.dur = '1s';

    return fullSpecs;
  }

  function generatePath(start, end, direction, borders, bend) {
